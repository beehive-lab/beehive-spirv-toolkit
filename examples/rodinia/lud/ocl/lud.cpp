...
static int initialize(int platform_id_inuse, int device_id_inuse)
{
	...
	int selected_platform_id = get_platform_with_2_1(all_platform_id, num_platforms);
	if (selected_platform_id < 0) {
        puts("ERROR: could not find OCL platform with version 2.1 or higher");
        return -1;
    }
	else if (selected_platform_id != platform_id_inuse) {
		puts("WARNING: changing selected platform since it does not support OpenCL 2.1");
		platform_id_inuse = selected_platform_id;
	}
    ...
    size_t name_size;
    if (clGetDeviceInfo(device_list[device_id_inuse], CL_DEVICE_NAME, 0, NULL, &name_size) != CL_SUCCESS) {puts("ERROR: could not retrieve name size"); return -1;}
    char *name = new char[name_size];
    if (clGetDeviceInfo(device_list[device_id_inuse], CL_DEVICE_NAME, name_size, name, &name_size) != CL_SUCCESS) {puts("ERROR: could notretrieve device name"); return -1;}
    printf("%s\n", name);    
...
main ( int argc, char *argv[] )
{
	...
	FILE *f = fopen("./lud_kernel.spv", "rb");
	fseek(f, 0, SEEK_END);
	long fsize = ftell(f);
	fseek(f, 0, SEEK_SET);
	...
	cl_program prog = clCreateProgramWithIL(context, source, fsize, &err);
	if(err != CL_SUCCESS) { printf("ERROR: clCreateProgramWithIL() => %d\n", err); return -1; }
	char clOptions[110];
